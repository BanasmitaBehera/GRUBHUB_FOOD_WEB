...
import Avatar from "../assests/download.jpg";
    ...
    <header className="bg-white fixed z-50 w-screen p-3 px-4 md:p-6 md:px-16">
        ...
        <div className="hidden md:flex w-full h-full items-center justify-between">
            <Link to={"/"} className="flex items-center gap-2">
                <img src={Logo1} className="w-10 object-cover" alt="logo" />
                <p className="text-headingColor text-xl font-bold"> Grubhub </p>
            </Link>
            ...
            className="flex items-center gap-8">
                <Link to="/login" className="text-base text-textColor hover:text-headingColor duration-100 transition-all ease-in-out cursor-pointer">
                <li >
                    Login
                </li>
                </Link>
        ...
        <div className="relative">
            ...
            {isMenu && (
                ...
                {user && user.email === "banasmitabehera306@gmail.com" &&  (
                    <Link to={"/createItem"}>
        ...
        <div className="relative">
            ...
            {isMenu && (
                ...
                exit={{opacity: 0, scale: 0.6}} 
                className="w-40 bg-gray-50 shadow-xl rounded-lg flex flex-col absolute top-12 right-0">
                {user && user.email === "banasnitabehera306@gmail.com" && (
                    <Link to={"/createItem"}>
                    <p className="px-4 py-2 flex items-center cursor-pointer
                    hover:bg-slate-100 transition-all duration-100 ease-in-out text-textColor text-base">New Item <MdAdd /></p>
                    </Link>
                    
                )}
                <p className="m-2 p-2 rounded-md shadow-md flex items-center justify-center
                bg-gray-300 gap-3 cursor-pointer hover:bg-slate-300 transition-all 
